...
//                                        alert(document.getElementById("getStock").value+ " --- "+v);
                        ...
                        <%--
                                    $(document).ready(init);
                                    function init() {
                                        $(".plus_btn").click(plus_btnHandler);
                                        $(".down_btn").click(down_btnHandler);
                                    }
                                    function plus_btnHandler() {
                                        var i = $(this).parent().find('input[class=quantity_box]');
                                        if (parseInt(i.val()) < <%=p.getStock() > 5 ? 5 : p.getStock()%>) {
                                            i.val(parseInt(i.val()) + 1);
                                        }
                                    }
                                    function down_btnHandler() {
                                        var i = $(this).parent().find('input[class=quantity_box]');
                                        i.val(parseInt(i.val()) - 1);
                                        if (parseInt(i.val()) < 1) {
                                            i.val(1);
                ...
                <div class="article" style="padding:  50px 0;">
                        ...
                        <div >
                                ...
                                <div style="float:left;width:65%">
                                        ...
                                        <p>原價: <%=Math.round(p.getUnitPrice())%></p>

                                        <div>優惠價：<%= Math.round(p.getPrice())%>元</div>

                                        <hr>
                                        <%= p.getDescription()%><!--產品介紹 -->
                                        <br><br><hr>
                                        <div style="">庫存量<input style="border: none;font-size: 20px;width: 30px;" id="getStock" value="<%=p.getStock()%>" readonly></div>
                                        <hr>
                                        ...
                                        <form action='add_cart.do' method="POST">
                                                ...
                                                <div id="zz">+</div>
<!--                                                <script>
                                                        $(document).ready(init);//ready--程式全部執行完後才觸發
                                                        function init() {

                                                                if
                                                                        $("#add_card").click(clickHandler);//所以就算按鈕程式碼寫在下方，按下時觸發，依然可以觸發    
                                                        }

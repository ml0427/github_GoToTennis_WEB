...
<body>
	...
	<jsp:include page="/WEB-INF/subviews/picture/small_menu.jsp">
		<jsp:param name="theSubtitle" value="<a class=\" small_menu_a\" href=\"/gtt\">首頁</a>>會員註冊   " />
                        <jsp:param name="h2_theSubtitle" value="會員註冊" /></jsp:include>
			...
			<form action="register.do" method="POST">
				<p>
					...
					<input id="email" name="email" value="${param.email}"
						placeholder="請輸入信箱" pattern="[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}"
						required>
				</p>
				...
				<p>
					...
					<input id="id" name="id" value="${param.id}" placeholder="請輸入身分證號"
				...
				<p>
					<label>性別:</label>
					<input style="height: auto; width: auto; float: left;" type="radio"
						id="male" name="gender" value="<%=Customer.MALE%>" required
					...
					<input style="height: auto; width: auto; float: left;" type="radio"
						id="female" name="gender" value="${Customer.FEMALE} %>"
						required ${param.gender==String.valueOf(Customer.FEMALE)?"checked":""} >
					<label style="height: auto; width: auto; float: left;" for="female">女</label>
				</p>

				<p style="clear: both;">
					<label for="birthday">生日:</label>
					<input type="date" max='<%= LocalDate.now()%>' id="birthday"
						value="${param.birthday}" name="birthday" >
				</p>

...
<link rel="stylesheet" type="text/css" href="<%=getURL%>/fancybox/jquery.fancybox.css">
<script src='<%=getURL%>/fancybox/jquery.fancybox.js'></script>
<%
...
<body>
	<div style="background-color: white;">
		...
		<ul style="">
			<li><a href="products_list.jsp?search=PRO+STAFF"> <img style="float: left; width: 47%; margin: 2%; background-color: white;" src="http://www.nandasports.com/images/banner/3300766656.jpg">
			</a></li>
			<li><a href="products_list.jsp?search=HEAD+Graphene+TOUCH+Speed"> <img style="float: right; width: 47%; margin: 2% 2% 2% 0; background-color: white;" src="http://www.nandasports.com/images/banner/5564815341.jpg">
			</a></li>
		</ul>
		...
		<div>
			<%
				...
				List<Product> list = service.selectProductByName(search); //"" 將會查詢到全部的產品
			%>
			<ul id="product_list">
				<%
					int new_product = 20;
				...
				<li style="height: 350px; box-shadow: none;">
					<div style="position: absolute; z-index: 1; right: 0; background-color: #6699ff; color: white; opacity: 0.7; width: 50px; height: 30px; line-height: 30px;">NEW</div> <a href="product.jsp?productId=<%=list.get(i).getId()%>" class="hover"> <img src="<%=list.get(i).getPhotoUrl()%>"> <img src="<%=list.get(i).getPhotoUrl2()%>">
				</a>

					<h5 style="padding: 234px 0 0 0;"><%=list.get(i).getName()%></h5>
					<div>
						<samp style="text-decoration: line-through;">
							原價$<%=Math.round(list.get(i).getUnitPrice())%></samp>
						<samp style="background-color: peachpuff;">
							優惠價$<%=Math.round(list.get(i).getPrice())%></samp>
					</div>
				</li>
				<%
					}

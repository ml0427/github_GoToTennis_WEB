...
<body>
	...
	<jsp:include page="/WEB-INF/subviews/picture/small_menu.jsp">
		<jsp:param name="theSubtitle" value="<a class=\" small_menu_a\" href=\"/gtt\">首頁</a>>會員修改   " />
                        <jsp:param name="h2_theSubtitle" value="會員修改" /></jsp:include>
	<%
		...
		<div class="article">
			...
			<form action="update.do" method="POST">
				<p>
					...
					<input id="id" name="id" value="<%=request.getParameter("id") == null ? member.getId() : request.getParameter("id")%>" readonly>
				</p>
				<p>
					<label for="name">姓名:</label>
					<input id="name" name="name" placeholder="請輸入姓名" required value="<%=request.getParameter("name") == null ? member.getName() : request.getParameter("name")%>">
				...
				<p>
					<label>性別:</label>
					<input style="height: auto; width: auto; float: left;" type="radio" id="male" name="gender" value="<%=Customer.MALE%>" required <%=request.getParameter("gender") != null && request.getParameter("gender").charAt(0) == Customer.MALE || request.getParameter("gender") == null && member.getGender() == Customer.MALE ? "checked" : ""%>>
				...
				<p style="clear: both;">
					<label style=""email">電郵:</label>
					<input type="email" id="email" value="<%=request.getParameter("email") == null ? member.getEmail() : request.getParameter("email")%>" name="email" placeholder="請輸入電子郵件" required>
				</p>
				<p>
					<label for="birthday">生日:</label>
					<input type="date" id="birthday" value="<%=request.getParameter("birthday") == null ? member.getBirthday() : request.getParameter("birthday")%>" name="birthday" required>
				</p>
				<p>
					<label for="phone">電話:</label>
					<input type="tel" id="phone" value="<%=request.getParameter("phone") == null ? member.getPhone() != null ? member.getPhone() : "" : request.getParameter("phone")%>" name="phone">
				</p>
				<p>
					<label for="address">地址:</label>
					<!--<textarea id="address" name="address">台北市</textarea>-->
					<input id="address" name="address" value="<%=request.getParameter("address") == null ? member.getAddress() != null ? member.getAddress() : "" : request.getParameter("address")%>">
				</p>
				<p>
